...
*   automaticamente o papel (role) de 'ADMIN' a um usuário específico (`admin@tricket.com.br`) no momento em
...
-- Criação da tabela para log de credenciais de contas Asaas
-- ⚠️  REMOVER EM PRODUÇÃO - uso temporário para debugging

CREATE TABLE IF NOT EXISTS debug_account_credentials (
  ...
  purpose TEXT DEFAULT 'DEBUG_ACCOUNT_CREDENTIALS',
  warning TEXT DEFAULT 'REMOVER_EM_PRODUCAO'
);

-- Índices para busca rápida
CREATE INDEX IF NOT EXISTS idx_debug_credentials_profile_id ON debug_account_credentials(profile_id);
CREATE INDEX IF NOT EXISTS idx_debug_credentials_asaas_id ON debug_account_credentials(asaas_account_id);
CREATE INDEX IF NOT EXISTS idx_debug_credentials_created_at ON debug_account_credentials(created_at DESC);

-- Comentários para lembrar de remover
COMMENT ON TABLE debug_account_credentials IS 'Tabela temporária para debugging de credenciais - REMOVER EM PRODUÇÃO';
COMMENT ON COLUMN debug_account_credentials.webhook_token IS 'Token para webhooks - armazenado temporariamente para debugging';
COMMENT ON COLUMN debug_account_credentials.api_key IS 'API Key criptografada - armazenada temporariamente para debugging';
