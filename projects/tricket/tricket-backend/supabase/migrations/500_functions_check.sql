...
-- 2. Cria a nova versão da função com a especificação de segurança correta
CREATE OR REPLACE FUNCTION public.check_user_has_role(p_role_name TEXT)
RETURNS BOOLEAN
LANGUAGE plpgsql
-- SECURITY INVOKER é a correção crucial. A função agora executa com as permissões do chamador.
SECURITY INVOKER
...
BEGIN
    ...
    SELECT EXISTS (
        ...
        WHERE ur.user_id = v_user_id AND r.name = p_role_name
    ) INTO v_has_role;
...
COMMENT ON FUNCTION public.check_user_has_role(TEXT) IS 'Função de segurança que verifica se o usuário autenticado possui um papel específico. Essencial para uso em políticas de RLS e validações de RPC. Executa com as permissões do chamador.';

-- A permissão de execução é para `public` porque a função é segura por design (apenas verifica o próprio chamador).
GRANT EXECUTE ON FUNCTION public.check_user_has_role(TEXT) TO public;
