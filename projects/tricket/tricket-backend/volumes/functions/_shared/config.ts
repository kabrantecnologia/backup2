...
export function loadConfig(): AppConfig {
  ...
  return {
    ...
    capptaApiUrl: Deno.env.get('CAPPTA_API_URL')!,
    capptaApiToken: Deno.env.get('CAPPTA_API_TOKEN')!,
    resellerDocument: Deno.env.get('RESELLER_DOCUMENT')!,
...
export function validateConfig(config: AppConfig): { isValid: boolean; errors: string[] } {
  ...
  try {
    new URL(config.capptaApiUrl);
  } catch {
    errors.push('CAPPTA_API_URL deve ser uma URL válida');
  }
  ...
  if (config.capptaApiToken.length < 10) {
    errors.push('CAPPTA_API_TOKEN parece inválido');
  }

  if (!config.resellerDocument || config.resellerDocument.length < 14) {
    errors.push('RESELLER_DOCUMENT deve ter pelo menos 14 caracteres');
  }
