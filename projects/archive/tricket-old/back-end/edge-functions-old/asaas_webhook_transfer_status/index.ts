// supabase/functions/webhook_transfer_status/index.ts
...
interface AsaasTransferWebhookPayload {
  id: string; // ID do evento Asaas (evt_...)
  event: string; // Tipo do evento Asaas (TRANSFER_*)
  dateCreated: string;
  transfer: {
    object: string;
    id: string;
...
serve(async (req: Request) => {
  ...
  try {
    ...
    let payload: AsaasTransferWebhookPayload;
    try {
      payload = await req.json();
      if (!payload.id || !payload.event || !payload.transfer || !payload.transfer.id) {
        logger.warn(`Payload inválido: faltando campos obrigatórios`, { payload });
        throw new Error("Missing required fields in webhook payload.");
      }
      
      // Validar se é um evento de transferência
      if (!payload.event.startsWith('TRANSFER_')) {
        logger.warn(`Tipo de evento inválido`, { eventType: payload.event, expected: 'TRANSFER_*' });
        throw new Error(`Invalid event type: ${payload.event}. Expected TRANSFER_* event.`);
      }
    ...
    logger.info(`Enfileirando evento de transferência`, { eventId: payload.id });
    ...
    if (insertError) {
      ...
      logger.info(`Evento de transferência enfileirado com sucesso`, { 
        eventId: payload.id, 
    ...
    return new Response(JSON.stringify({ message: 'Transfer webhook received and enqueued for processing', eventId: payload.id }), {
      status: 200,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
    ...
    logger.critical(`Exceção não tratada no receptor de webhook de transferências`, { 
      errorId, 
      message: e.message, 
      stack: e.stack 
    });
