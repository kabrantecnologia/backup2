...
serve(async (req) => {
  logger.info('Requisição de registro de webhook recebida', { 
    method: req.method, 
    url: req.url 
  });
  ...
  try {
    ...
    try {
      ...
      if (!payload.profile_id || !payload.webhook_type || !payload.webhook_url) {
        logger.warn('Dados inválidos no payload', { payload });
        return new Response(
          JSON.stringify({ error: 'Dados inválidos', details: 'profile_id, webhook_type e webhook_url são obrigatórios' }),
    ...
    const { profile_id, webhook_type, webhook_url } = payload;
    ...
    // Registrar na tabela cappta_webhooks
    const { data: insertedWebhook, error: insertError } = await supabase
      ...
      .upsert({
        ...
        onConflict: 'profile_id,webhook_type',
    ...
    if (insertError) {
      logger.error('Erro ao registrar webhook', { error: insertError.message });
      return new Response(
        JSON.stringify({ error: 'Erro interno ao registrar webhook', details: insertError.message }),
        { status: 500, headers: { 'Content-Type': 'application/json' } }
      );
    }
    
    // A API do Cappta pode ser chamada aqui para registrar o webhook no lado deles, se necessário
    // Usando as variáveis CAPPTA_API_KEY e CAPPTA_API_URL do vault
    ...
    return new Response(
      JSON.stringify({
        ...
        data: {
          ...
          profile_id: insertedWebhook.profile_id,
          webhook_type: insertedWebhook.webhook_type,
          webhook_url: insertedWebhook.webhook_url,
          webhook_token: insertedWebhook.webhook_token,
          is_active: insertedWebhook.is_active,
          created_at: insertedWebhook.created_at
        }
      }),
