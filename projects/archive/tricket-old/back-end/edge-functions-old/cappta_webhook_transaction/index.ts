...
serve(async (req) => {
  logger.info('Requisição de webhook de transação recebida', { 
    method: req.method, 
    url: req.url 
  });
  ...
  try {
    ...
    try {
      ...
      logger.info('Payload do webhook de transação recebido', { 
        payload: payload,
      ...
      const { data: insertedEvent, error: insertError } = await supabase
        ...
        .insert({
          ...
          event_id: payload.transactionId || payload.id || null,
          payload: payload,
      ...
      if (insertError) {
        logger.error('Erro ao salvar evento de transação', { error: insertError.message });
        return new Response(
      ...
      logger.info('Evento de transação salvo com sucesso', { eventId: insertedEvent.id });
      
      return new Response(
        JSON.stringify({ success: true, message: 'Webhook de transação recebido com sucesso' }),
    ...
    logger.error('Erro ao processar webhook de transação', { 
      error: error.message, 
      stack: error.stack 
    });
