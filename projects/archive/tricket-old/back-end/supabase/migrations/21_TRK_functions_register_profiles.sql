...
*   NOME DO ARQUIVO: 11_functions_register_profiles.sql
*   VERSÃO: 1.0
*   CRIADO POR: Gemini
*   DATA DE CRIAÇÃO: 2025-07-25
*
*   -- SUMÁRIO --
*   Este script cria as funções RPC (Remote Procedure Call) para o registro de novos perfis na plataforma,
*   abrangendo tanto perfis de organização (pessoa jurídica) quanto perfis individuais (pessoa física).
*   As funções são projetadas para serem chamadas a partir do frontend ou de um backend intermediário,
*   consolidando toda a lógica de criação de entidades relacionadas (detalhes, endereços, membros) em uma
*   única transação atômica, garantindo a consistência dos dados.
*
**********************************************************************************************************************/
...
BEGIN
    ...
    IF NOT FOUND THEN
        ...
        VALUES (v_representative_pf_profile_id, v_auth_user_id, (COALESCE(individual_data->>'profile_role', 'COLLABORATOR'))::public.individual_profile_role_enum, individual_data->>'full_name', individual_data->>'cpf', (individual_data->>'birth_date')::DATE, individual_data->>'contact_phone', individual_data->>'contact_email');
    ELSE
        ...
        UPDATE public.iam_individual_details SET profile_role = (COALESCE(individual_data->>'profile_role', profile_role::TEXT))::public.individual_profile_role_enum, full_name = COALESCE(individual_data->>'full_name', full_name), cpf = COALESCE(individual_data->>'cpf', cpf), birth_date = COALESCE((individual_data->>'birth_date')::DATE, birth_date), contact_phone = COALESCE(individual_data->>'contact_phone', contact_phone), contact_email = COALESCE(individual_data->>'contact_email', contact_email), updated_at = now() WHERE profile_id = v_representative_pf_profile_id;
    END IF;
    ...
    VALUES (v_organization_profile_id, (organization_data->>'platform_role')::public.organization_platform_role_enum, organization_data->>'company_name', organization_data->>'trade_name', organization_data->>'cnpj', (organization_data->>'company_type')::public.company_type_enum, COALESCE((organization_data->>'income_value_cents')::BIGINT, ((organization_data->>'income_value')::NUMERIC(15,2) * 100)::BIGINT), organization_data->>'contact_email', organization_data->>'contact_phone');
...
BEGIN
    ...
    VALUES (v_new_profile_id, v_auth_user_id, profile_data->>'full_name', profile_data->>'cpf', (profile_data->>'birth_date')::DATE, profile_data->>'contact_email', profile_data->>'contact_phone', (COALESCE(profile_data->>'profile_role', 'CONSUMER'))::public.individual_profile_role_enum, (profile_data->>'income_value_cents')::BIGINT);
