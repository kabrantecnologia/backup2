...
*   NOME DO ARQUIVO: 08_function_get_user_contexts.sql
*   VERSÃO: 1.0
*   CRIADO POR: Gemini
...
-- Remove a função anterior, se existir, para garantir uma nova implementação limpa.
DROP FUNCTION IF EXISTS public.get_user_contexts();
...
/**********************************************************************************************************************
*   SEÇÃO 2: FUNÇÃO DE NAVEGAÇÃO DA UI
*   Descrição: Constrói a estrutura de navegação completa para o usuário com base em suas permissões.
**********************************************************************************************************************/

-- Função: get_navigation_for_user
-- Retorna uma estrutura JSON com o menu principal (sidebar) e os menus de página (abas) aninhados,
-- filtrados de acordo com as permissões do usuário autenticado.
...
BEGIN
    ...
    WITH RECURSIVE accessible_elements AS (
        -- Elementos base: menus e abas diretamente permitidos para os papéis do usuário.
        SELECT el.*
        ...
        UNION
        -- Recursão: inclui os elementos pai dos itens acessíveis para montar a árvore completa.
        SELECT el.*
        FROM public.ui_app_elements el
        INNER JOIN accessible_elements ae ON el.id = ae.parent_id
    ),
    page_menus AS (
        -- Agrupa as abas (PAGE_TAB) por página para fácil associação.
        SELECT p.id AS page_id, jsonb_agg(jsonb_build_object('id', el.id, 'label', el.label, 'icon', el.icon, 'path', el.path) ORDER BY el.position) AS tabs
    ...
    menu_tree AS (
        -- Monta a árvore final do menu principal (SIDEBAR_MENU).
        SELECT
            el.id, el.label, el.icon, el.path, el.parent_id, el.position, pm.tabs AS page_menu,
            (SELECT jsonb_agg(sub_items ORDER BY sub_items->>'position') FROM (
                SELECT jsonb_build_object(
                    'id', sub.id, 'label', sub.label, 'icon', sub.icon, 'path', sub.path, 'position', sub.position,
                    'pageMenu', COALESCE(sub_pm.tabs, '[]'::jsonb)
                ) AS sub_items
                FROM accessible_elements sub
                LEFT JOIN page_menus sub_pm ON sub_pm.page_id = (SELECT id FROM public.ui_app_pages WHERE path = sub.path)
                WHERE sub.parent_id = el.id AND sub.element_type = 'SIDEBAR_MENU'
            ) AS sub_query) AS "subItems"
        FROM accessible_elements el
        LEFT JOIN page_menus pm ON pm.page_id = (SELECT id FROM public.ui_app_pages WHERE path = el.path)
        WHERE el.parent_id IS NULL AND el.element_type = 'SIDEBAR_MENU'
    )
    -- 3. Agrega o resultado final em um único array JSON.
    SELECT jsonb_agg(jsonb_build_object(
        'id', mt.id, 'icon', mt.icon, 'label', mt.label, 'path', mt.path, 'isOpened', false,
        'pageMenu', COALESCE(mt.page_menu, '[]'::jsonb), 'subItems', COALESCE(mt."subItems", '[]'::jsonb)
...
COMMENT ON FUNCTION public.get_navigation_for_user() IS 'Retorna uma estrutura JSON completa com o menu principal e os menus de página (abas) aninhados, de acordo com as permissões do usuário.';