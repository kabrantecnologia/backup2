...
*   automaticamente o papel (role) de 'ADMIN' a um usuário específico (`testes@kabran.com.br`) no momento em
...
BEGIN
    ...
    IF NEW.email = 'testes@kabran.com.br' THEN
...
FOR EACH ROW
WHEN (NEW.email = 'testes@kabran.com.br') -- Otimização: o gatilho só é chamado para o email específico.
...
DECLARE
    ...
    admin_role_id UUID;
BEGIN
    SELECT id INTO target_user_id FROM auth.users WHERE email = 'testes@kabran.com.br';
    IF target_user_id IS NULL THEN RETURN 'ERRO: Usuário testes@kabran.com.br não encontrado.'; END IF;
    ...
    RETURN 'SUCESSO: Papel de ADMIN atribuído manualmente para testes@kabran.com.br.';
END;
$$;
