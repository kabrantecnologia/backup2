...
async function fetchAsaasAccount(
  ...
  if (data.account_status !== 'ACTIVE') {
    logger.warn('Account is not active', { requestId, profileId, status: data.account_status });
    throw new Error(`Account for profile ${profileId} is not active (status: ${data.account_status}).`);
  }
...
async function handleRequest(request: Request): Promise<Response> {
  ...
  // 2. Authentication
  const authResult = await authMiddleware(request, supabase, ['ADMIN', 'SUPER_ADMIN']);
