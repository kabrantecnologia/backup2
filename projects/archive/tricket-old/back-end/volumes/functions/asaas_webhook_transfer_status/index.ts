...
async function handleRequest(request: Request): Promise<Response> {
  ...
  if (!webhookToken || webhookToken !== Deno.env.get('ASAAS_WEBHOOK_AUTH_TOKEN')) {
    logger.warn('Unauthorized access attempt', { requestId, hasToken: !!webhookToken });
    return createAuthErrorResponse('Authentication token is invalid or missing.');
  }
  ...
  const { error: insertError } = await supabase
    ...
    .insert({
      ...
      status: 'received',
      idempotency_key: `asaas-transfer-${payload.transfer.id}-${payload.event}`,
    });
